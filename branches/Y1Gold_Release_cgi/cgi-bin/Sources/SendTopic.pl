...
sub SendTopic
{
	...
	fopen(FILE, "$datadir/$topic.txt") || &fatal_error("201 $txt{'106'}: $txt{'23'} $threadid.txt");
	@messages = <FILE>;
	fclose(FILE);
...
sub SendTopic2 {
	...
	&fatal_error($txt{'243'}) if($yemail !~ /^[0-9A-Za-z@\._\-]+$/);
	...
	&fatal_error($txt{'243'}) if($remail !~ /^[0-9A-Za-z@\._\-]+$/);
