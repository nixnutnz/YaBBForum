...
~;
    ...
    <span class="small"><b>		   
	    <a href="$scripturl" class="nav">$mbname</a> » 
          <a href="$scripturl?catselect=$catid" class="nav">$cat</a> » 
		     <a href="$scripturl?board=$currentboard" class="nav">$boardname</a> »
...
~;
	unless ($pollthread == 2) { $yymain .= qq~		<input type="submit" name="$preview" value="$post_txt{'507'}" accesskey="p" tabindex="6" />~; }
	unless ($is_preview)      { $yymain .= qq~       <input type="reset" value="$post_txt{'278'}" accesskey="r" tabindex="7" />~; }
	$yymain .= qq~
        </td>
      </tr>
</table>
