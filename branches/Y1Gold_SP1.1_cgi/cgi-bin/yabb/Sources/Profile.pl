...
~;
	...
	<BR>$txt{'741'}: <i>$proftime</i></font></td>
		...
		<BR><td colspan=2><hr width="100%" size="1" class="hr"></td>
      </tr><tr>
...
~;
    ...
    <input type=submit name=moda value="$txt{'89'}" onClick="return confirmDel()"><BR><BR>
    </font></td>
  </tr>
</table>
</form>
...
sub ModifyProfile2 {
	...
	if($member{'moda'} eq $txt{'88'}) {
		...
		for ($a = 0; $a < @memberlist; $a++) {
			...
			if( fopen(FILE2, "$memberdir/$memberlist[$a].dat") ) {
				# Load users and check email (removed !$yyUDLoaded{$memberlist[$a]} && )
				if( -e("$memberdir/$memberlist[$a].dat") && lc $memberlist[$a] ne lc $member{'username'}) {
				...
				if ($realnametemp eq $testname && lc $memberlist[$a] ne lc $member{'username'} && $settings[7] ne 'Administrator') { &fatal_error("($member{'name'}) $txt{'473'}"); }
			}
		}
	...
	exit;
		...
		if ($member{'username'} eq $username) {
			$password = crypt("$member{'passwrd1'}",$pwseed);
			$yySetCookies1 = cookie(-name    =>   "$cookieusername",
						-value   =>   "$username",
						-path    =>   "/",
