...
sub LoadUserDisplay {
	...
	if($sm == 1) { $userprofile{$user}->[4] = $userprofile{$user}->[4] && $userprofile{$user}->[4] ne q~http://~ ? qq~ <a href="$userprofile{$user}->[4]" target="_blank">$img{'website_sm'}</a>$menusep~ : ''; }
	...
	$userprofile{$user}->[5] = $userprofile{$user}->[5] ? qq~<hr width="100%" size="1" color="$color{'windowbg3'}"><font size="1">$userprofile{$user}->[5]</font>~ : '';
	# do some ubbc on the signature
	$message = $userprofile{$user}->[5];
	if($enable_ubbc) { &DoUBBC; }
	$userprofile{$user}->[5] = $message;

	if( $userprofile{$user}->[8] && $userprofile{$user}->[8] !~ m~\D~ ) {
		...
		$userprofile{$user}->[8] = qq~<a href="$cgi&action=icqpager&UIN=$userprofile{$user}->[8]" target="_blank"><img src="http://wwp.icq.com/scripts/online.dll?icq=$userprofile{$user}->[8]&img=5" alt="$userprofile{$user}->[8]" border="0"></a>~;
	}
	else {
		$icqad{$user} = '';
		$userprofile{$user}->[8] = '';
	...
	if( $userprofile{$user}->[10] ) {
		$yimon{$user} = qq~<img SRC="http://opi.yahoo.com/online?u=$userprofile{$user}->[10]&m=g&t=0" NOSAVE BORDER=0 alt="$userprofile{$user}->[10]">~;
	...
	if( $showuserpic && $allowpics) {
		...
		$userprofile{$user}->[13] = $userprofile{$user}->[13] =~ m~\A[\s\n]*http://~i ? qq~<br><img src="$userprofile{$user}->[13]"$userpic_tmpwidth$userpic_tmpheight border="0"><br><br>~ : qq~<br><img src="$facesurl/$userprofile{$user}->[13]" border="0"><br><br>~;
	}
	else {
		$userprofile{$user}->[13] = '<BR>';
	}
