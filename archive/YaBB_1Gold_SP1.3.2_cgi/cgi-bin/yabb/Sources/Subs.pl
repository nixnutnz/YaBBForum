...
sub redirectexit {
	print header(-status=>"302 Moved Temporarily", -location=>$yySetLocation);
	exit;
}
...
sub template {
	if($yySetCookies1 || $yySetCookies2) { print header(-status=>"200 OK", -cookie=>[$yySetCookies1, $yySetCookies2], -charset=>$yycharset); }
...
sub wrap {
	...
	foreach $cur (@words) {
		if($cur !~ m~[ht|f]tp://~ && $cur !~ m~\[\S*\]~ && $cur !~ m~\[\S*\s?\S*?\]~ && $cur !~ m~\[\/\S*\]~) {
			...
			if($sender eq "search") {
				foreach( @search ) {
					if($cur !~ m~<yabbbr>~) { $cur =~ s~(\Q$_\E)~\[b\]$_\[/b\]~ig; }
				}
			}
		}
