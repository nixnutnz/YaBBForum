...
If you're testing or upgrading by overwriting the files in Admin, Sources, Languages, etc. be sure to also replace index.pl and AdminIndex.pl. And, most importantly, rename your Settings.pl to Settings.pm and Paths.pl to Paths.pm. 
...
*NEW instructions for upgrading.* We are now using the Convert utilities for *all* upgrades - even from 2.5.2. If your old Forum is NOT in the same cgi-bin as your new YaBB installation OR you do not know the server path to your old YaBB files, copy your old Variables, Members, Messages and Boards folder contents into their appropriate Convert folders in 2.6.14. Convert.pl (for 1x versions of YaBB) or Convert2x.pl (for 2x versions of YaBB) will double check the folder permissions and copy the files over for you - this removes some confusion as to exactly which old files need to be copied into Variables and also allows for the old settings in Settings.pl to be imported into the new Settings.pm. This is *also* in preparation for future changes in data formatting *and* as a check on YaBB's read/write permissions on the server.
