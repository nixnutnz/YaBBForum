	...
	foreach $curcat (@categories) {
		...
		foreach $curboard (@catboards ) {
			...
			if($recentsender eq "admin") {
				...
				$post = qq~<BR>$txt{'235'}: <a href="$cgi$curboard;action=display;num=$mnum;start=$mreplies"><B>$msub</B></a> ($mydatetime)<BR>$txt{'791'} <A href="$scripturl?action=recent">$txt{'792'}</a> $txt{'793'}~;
			}
